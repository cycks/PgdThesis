...
try:
    ...
    extraction_dir = './shared_komnet_data' # UPDATED
    image_size = (256, 256)
    batch_size = 32

    # 1. Data Handling (Check for existing folder before unzipping)
    if not os.path.exists(extraction_dir):
        if os.path.exists(zip_filename):
            print(f"Directory {extraction_dir} not found. Extracting {zip_filename}...")
            with zipfile.ZipFile(zip_filename, 'r') as zip_ref:
                zip_ref.extractall(extraction_dir)
            print("Extraction complete.")
        else:
            print(f"CRITICAL ERROR: Neither {extraction_dir} nor {zip_filename} exist.")
            sys.exit(1)
    else:
        print(f"Using existing data found in: {extraction_dir}")

    # 2. Load Datasets
    rescale = tf.keras.Sequential([layers.Rescaling(1./255)])
    
    train_ds = tf.keras.utils.image_dataset_from_directory(
        extraction_dir, # UPDATED
