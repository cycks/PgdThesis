...
def save_performance_plots(history):
    ...
    plt.savefig('cycks_with_padding_accuracy_loss_plots.png')
    print("Saved cycks_with_padding_accuracy_loss_plots.png")
...
def save_roc_curve(y_true, y_probs, num_classes):
    ...
    plt.legend(loc="lower right")
    plt.savefig('cycks_with_padding_roc_curve.png')
...
# --- Variable Importance (Saliency Map) ---
def save_saliency_map(model, dataset, class_names):
    # 1. Grab one batch from the validation set
    for images, labels in dataset.take(1):
        img = images[0:1]  # Take the first image of the batch
    ...
    # Calculate gradients of the top class score with respect to the input image
    grads = tape.gradient(top_class_score, img_tensor)
    
    # Take the absolute maximum across color channels
    ...
    plt.savefig('cycks_with_padding_variable_importance.png')
    print("Saved cycks_with_padding_variable_importance.png")
...
save_saliency_map(model, val_ds, class_names)
