...
def save_performance_plots(history):
    ...
    plt.title('Accuracy: Cycks Without Padding')
    ...
    plt.title('Loss: Cycks Without Padding')
...
def save_variable_importance(model, dataset, class_names):
    ...
    # 2. Set up the Gradient Tape to watch the input image
    img_tensor = tf.convert_to_tensor(img)
    with tf.GradientTape() as tape:
        tape.watch(img_tensor)
        predictions = model(img_tensor)
        # We target the class the model is most confident in
        top_class_index = tf.argmax(predictions[0])
        top_class_score = predictions[:, top_class_index]

    # 3. Calculate gradients: How much does each pixel affect the final score?
    grads = tape.gradient(top_class_score, img_tensor)
    
    # Process gradients for visualization (absolute value and max across channels)
    ...
    plt.tight_layout()
    plt.savefig('cycks_without_padding_variable_importance.png')
    print("Saved cycks_without_padding_variable_importance.png")
...
# --- Final Execution ---
save_variable_importance(model, val_ds, class_names)