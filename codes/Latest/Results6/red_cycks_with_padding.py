...
# --- LOGGING SETUP ---
log_file_path = "Red_cycks_With_Padding_Log.txt"
...
def save_performance_plots(history):
    ...
    plt.title('Training and Validation Accuracy for Red_cycks_With_Padding')
    ...
    plt.savefig('Red_cycks_with_padding_accuracy_loss_plots.png')
    print("Saved Red_cycks_with_padding_accuracy_loss_plots.png")
...
# --- Variable Importance (Saliency Map) Implementation ---

def save_saliency_map_visual(model, dataset, class_names):
    ...
    with tf.GradientTape() as tape:
        ...
        # Target the top predicted class score
        top_class_index = tf.argmax(predictions[0])
        top_class_score = predictions[:, top_class_index]

    # Calculate gradient of top score with respect to input pixels
    grads = tape.gradient(top_class_score, img_tensor)
    
    # Process for visualization: absolute value and max across RGB channels
    saliency = tf.reduce_max(tf.abs(grads), axis=-1)[0]
    
    # Normalize the saliency map for better display
    ...
    plt.subplot(1, 2, 1)
    plt.imshow(img[0])
    plt.title(f"Original User: {class_names[label_idx]}")
    plt.axis('off')

    # Saliency Map (Variable Importance)
    plt.subplot(1, 2, 2)
    plt.imshow(saliency, cmap='viridis') # 'viridis' or 'hot' work best here
    plt.title("Variable Importance (Saliency Map)")
    plt.colorbar(label='Influence Level')
    plt.axis('off')

    plt.tight_layout()
    plt.savefig('red_cycks_variable_importance.png')
    print("Saved red_cycks_variable_importance.png")
...
# --- Final Step in Execution ---
save_saliency_map_visual(model, val_ds, class_names)