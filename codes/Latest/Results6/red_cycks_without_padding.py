...
# --- LOGGING SETUP ---
log_file_path = "Red_cycks_Without_Padding_Log.txt"
...
def create_model(num_classes, input_shape=(256, 256, 3)):
    model = models.Sequential(name="Red_Cycks_Without_Padding")
...
def save_performance_plots(history):
    ...
    plt.title('Training and Validation Accuracy for Red_cycks_without_padding')
    ...
    plt.title('Training and Validation Loss for Red_cycks_without_padding')
    plt.legend(loc='upper right')
    
    plt.savefig('Red_cycks_without_padding_accuracy_loss_plots.png')
    print("Saved Red_cycks_without_padding_accuracy_loss_plots.png")
...
def save_roc_curve(y_true, y_probs, num_classes):
    ...
    plt.title('Multi-class ROC Curve for Red_cycks_without_padding')
    plt.legend(loc="lower right")
    plt.savefig('Red_cycks_without_padding_roc_curve.png')
...
def save_red_cycks_saliency(model, dataset, class_names):
    # 1. Grab a single batch and extract one image
    for images, labels in dataset.take(1):
        img = images[0:1]  # Select the first image
        label_idx = labels[0].numpy()
        break

    # 2. Compute the Saliency Map
    ...
    # Calculate the gradients of the top class score w.r.t. the input image
    grads = tape.gradient(top_class_score, img_tensor)
    
    # Take the absolute value and the maximum across the RGB channels
    ...
    plt.subplot(1, 2, 2)
    plt.imshow(saliency, cmap='magma') # 'magma' provides high contrast for importance
    plt.title("Variable Importance (Saliency Map)")
    plt.colorbar(fraction=0.046, pad=0.04)
    plt.axis('off')

    plt.tight_layout()
    plt.savefig('red_cycks_without_padding_saliency.png')
    print("Saved red_cycks_without_padding_saliency.png")
...
# --- Final Execution ---
save_red_cycks_saliency(model, val_ds, class_names)
