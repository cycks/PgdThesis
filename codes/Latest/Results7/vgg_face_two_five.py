...
# --- LOGGING SYSTEM ---
log_file_path = "VGG_No_Padding_Five_Log.txt"
...
def create_model(num_classes_arg, input_shape_arg=(256, 256, 3)):
    ...
    # Block 1: (256x256) -> (254x254) -> (252x252) -> (126x126)
    model.add(layers.Conv2D(64, (3, 3), activation='relu', input_shape=input_shape_arg, padding='valid'))
    model.add(layers.Conv2D(64, (3, 3), activation='relu', padding='valid'))
    model.add(layers.MaxPooling2D((2, 2), strides=(2, 2)))
    
    # Block 2: (126x126) -> (124x124) -> (122x122) -> (61x61)
    model.add(layers.Conv2D(128, (3, 3), activation='relu', padding='valid'))
    model.add(layers.Conv2D(128, (3, 3), activation='relu', padding='valid'))
    model.add(layers.MaxPooling2D((2, 2), strides=(2, 2)))
    
    # Block 3: (61x61) -> (59x59) -> (57x57) -> (55x55) -> (27x27)
    model.add(layers.Conv2D(256, (3, 3), activation='relu', padding='valid'))
    model.add(layers.Conv2D(256, (3, 3), activation='relu', padding='valid'))
    model.add(layers.Conv2D(256, (3, 3), activation='relu', padding='valid'))
    model.add(layers.MaxPooling2D((2, 2), strides=(2, 2)))
    
    # Block 4: (27x27) -> (25x25) -> (23x23) -> (21x21) -> (10x10)
    model.add(layers.Conv2D(512, (3, 3), activation='relu', padding='valid'))
    model.add(layers.Conv2D(512, (3, 3), activation='relu', padding='valid'))
    model.add(layers.Conv2D(512, (3, 3), activation='relu', padding='valid'))
    model.add(layers.MaxPooling2D((2, 2), strides=(2, 2)))
    
    # Block 5: (10x10) -> (8x8) -> (6x6) -> (4x4) -> (2x2)
    # This is the physical limit for 3x3 kernels on this input size
    model.add(layers.Conv2D(512, (3, 3), activation='relu', padding='valid'))
    model.add(layers.Conv2D(512, (3, 3), activation='relu', padding='valid'))
    model.add(layers.Conv2D(512, (3, 3), activation='relu', padding='valid'))
    model.add(layers.MaxPooling2D((2, 2), strides=(2, 2)))
    ...
    # Classification Head
    # Flattening a 2x2x512 = 2,048 vector
...
# --- Plotting Functions ---
def save_training_plots(history, acc_filename="VGG_No_Padding_Five_Accuracy_Loss.png"):
