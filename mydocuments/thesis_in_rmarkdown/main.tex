...
\section{Justification of the study}\label{justification-of-the-study}
...
\section{Facial Recognition}\label{facial-recognition}
...
\subsubsection{\texorpdfstring{Challenges in Face Detection\\
...
\subsubsection{\texorpdfstring{Face Detection Approaches\\
...
\subsubsection{\texorpdfstring{Template Matching / Geometric
Approaches\\
...
\subsubsection{\texorpdfstring{Appearance Based Methods\\
...
\section{Types of Neural Networks}\label{types-of-neural-networks}
...
\subsection{Multiple Layer Neural
...
\subsubsection{\texorpdfstring{Padding in Convolution Neural Networks\\
...
{\centering \includegraphics[width=0.8\linewidth]{img/zero_padding} 
...
{\centering \includegraphics[width=0.8\linewidth]{img/reflection_padding} 
...
{\centering \includegraphics[width=0.8\linewidth]{img/replication_padding} 
...
{\centering \includegraphics[width=0.8\linewidth]{img/cricular_padding} 
...
{\centering \includegraphics[width=0.8\linewidth]{img/sequence_padding} 
...
\section{Facial Recognition Datasets}\label{facial-recognition-datasets}
...
\subsection{Disguised Faces in The Wild
...
\subsection{VGGFACE}\label{vggface}
...
\subsection{KomNET Face Dataset}\label{komnet-face-dataset}
...
\subsection{Google FaceNet}\label{google-facenet}
...
\subsection{VGGFACE-16}\label{vggface-16}

VGGFACE-16 is a facial recognition that was developed by Karen Simonyan
and Andrew Zisserman from the University of Oxford (Koo et al., 2018).
It comprises multiple sequential connected layers with each layer
receiving output from the previous layer. Specifically, it is made up of
16 layers where the first thirteen layers are convolution layers while
the last three layers are fully connected layers. Critical to the
discussion is the fact that the input image must have a dimension of 224
by 224 by 3. The sixteen layers used by this facial architecture can be
divided into five major groups. Each of these groups contains
convolution layers, a relu activation function, maxpooling, kernel
filters, strides and padding. A thorough explanation of this facial
recognition architecture has been published by Koo et al., (2018). It is
important to mention that like Google Facenet and DeepFace, VGGFACE-16
was also tested on famous datasets such as the Labeled Faces in the Wild
dataset. Specifically, the model recorded an accuracy of 99.13\% on the
...
~~~~~~~This section discusses the theory behind neural networks by
deriving the back propagation algorithm. The section also discusses
techniques used in convolution neural networks by describing how CNNs
extract data from images. Specifically, this section focuses on major
techniques used in CNNs such as filtering, strides, padding, pooling and
batch normalization.

\section{Artificial Neural Networks}\label{artificial-neural-networks}

~~~~~~~Artificial neural networks is a machine learning technique that
tries to imitate the intelligence of a human brain. The human brain
contains neurons that are connected to one another through axons and
dendrites. The connecting region between axons and dendrites is called a
synapse. The strength of synaptic connections often changes due to
changes in external stimuli, which in turn influence how learning occurs
in the human brain (Agrawal and Singh, 2015). Like biological neurons,
artificial neural networks contain computational units called neurons.
These computational units are connected to one another through weights
that perform the same role as synapses in biological neurons. An
artificial neural network computes a function of the inputs by
propagating the computed values from the input neurons to the output
neuron(s) and using the weights as intermediate parameters. Learning
occurs by changing the weights connecting the neurons.
...
\caption{Artificial Neural Network.}\label{fig:ann}
\end{figure}

~~~~~~~The simplest form of a neural network is called a perceptron and
includes a single input layer, an activation function, and the output
layer. Consider the image shown in figure 1.0 above and the case of a
binary response variable \((Y)\) with 1 is the presence of an observed
parameter and 0 the absence of the observed parameter. The input layer
contains predictor variables \((X_1, X_2, …, X_n)\) with weights
\((W_1, W_2, \ldots, W_n)\), which are mapped to the output node Y using
an activation function. The activation function takes in the computed
probability of the function of the weights \(W_i\) and predictor
variable \(X_i\) and produces a probability of the response variable
being 1 or 0. The activation function then takes the predicted
probability and produces an output 1 if the predicted probability is
close to 1 and produces an output 0 if the predicted probability is
close to 1. It follows that the perceptron neural network can be
computed using the formula in equation 3.1 below.
...
~~~~~~~While equation 3.1 above may work in a variety of settings, there
are instances where we need to introduce a bias to a neural network for
it to work accordingly (Agrawal and Singh, 2015). The mentioned bias
could be compared to penalizing a regression model. Basically, we
introduce a bias neuron that shifts the line of fit to the right or left
depending on the original position of the line of fit. Another way to
think about biases is simply by considering any linear function, y = mx
+ b. Let's say you are using y to approximate some linear function z. If
z has a non-zero z-intercept, and you have no bias in the equation for y
(i.e.~y = mx), then y can never perfectly fit z. Similarly, if the
neurons in your network have no bias terms, then it can be harder for
your network to approximate some functions. The figure 3.2 below shows
an example of neural network architecture with a single bias neuron.

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{img/ann_with_bias} 

}

\caption{Artificial Neural Network.}\label{fig:ann_with_bias}
\end{figure}

Thus, the perceptron of a single layer neural network is shown in the
formula 3.2 below. The output is represented by \(Y_hat\), which is
equal to the sum of weight \(W(W_1, W_2, …, W_n)\) multiplied by the
predictor variables \(X(X_1, X_2, …, X_n)\) plus the bias neuron \(b\).
\begin{equation}
\begin{aligned}
...
\section{Deriving the Backpropagation
Algorithm}\label{deriving-the-backpropagation-algorithm}

Let \(W_{ji}^L\) be the weight matrices denoting the value of the
synaptic weight that connects the \(j^{th}\) neuron of layer \((L)\) to
the \(i^{th}\) neuron of layer \((L − 1)\). Consider the image in the
figure 3.3 below
...
{\centering \includegraphics[width=0.8\linewidth]{img/ann_proof} 

}

\caption{Artificial Neural Network.}\label{fig:ann_proof}
\end{figure}

Let \(I_{j}^L\) be vectors whose elements denote the weighted inputs of
the \(j^{th}\) neuron of layer \(L\) and are defined as the following.
...
Let \(Y_{j}^L\) be vectors whose elements denote the output of the
\(j^{th}\) neuron related to the layer \(L\) and are defined as:
...
Another important aspect of the backpropagation is the chain rule, which
is defined as: \(\frac{d}{dx} f(g(x)) = f'(g(x)) \cdot g'(x)\). The next
step of deriving the backpropagation algorithm involves defining a
function that can represent the approximation error. It is important to
highlight that this function will measure the deviation between the
actual values and the predicted values. As a result, the squared error
function is employed to measure the deviation in question. This error
function will be defined as shown in equation 3.9 below.
...
Where \(y_{j}^3 (k)\) is the value produced by the \(j^{th}\) output
neuron of the network for the \(k^{th}\) training sample, while
\(d_j (k)\) is the corresponding desired value. If there are \(p\)
training samples the performance of the backpropagation algorithm would
be calculated using the mean squared error defined by
...
Where \(E(k)\) is the squared error obtained in equation 3.9. In order
to communicate the derivation of the backpropagation algorithm the
process will be divided into two parts. The first part will focus on the
synaptic adjustment of weight matrix \(w_{ji}^(3)\), which represents
the output layer. The second part will focus on adjusting intermediate
layers \(w_{ji}^(2)\) and \(w_{ji}^(1)\).

\subsection{Adjusting the Synaptic Weights of the Output
Layer}\label{adjusting-the-synaptic-weights-of-the-output-layer}

~~~~~~~Prior to diving into the mathematics of deriving the
backpropagation algorithm, it should be noted that the purpose of the
training process in output layer is to minimize the error between
predicted and actual values. This will be achieved by minimizing the
difference between the predicted and actual values. In short, the
objective is to obtain an optimal \(w^*\) so that the squared error
\({E(w^*)}\) of the whole sample set is as low as possible. Using the
chain rule, we differentiate equation 3.9 as shown in equation 3.11
below.
...
Obtained from the partial derivative of equation 3.5 with respect to
\(W_{ji}\)
...
Obtained from equation 3.8. Where \(g'\) denotes the first order
...
Obtained from equation 3.9. Replacing equations 3.12, 3.13 and 3.14 in
...
Also, note that the direction of the weight \(W_{ji}^{(3)}\) matrix must
be made in the opposite direction of the gradient because the
optimization goal is to minimize the squared error (Da Silva et al.,
2017).
...
In short, we have shown through equation 3.17 that the neuron adjusts
weights of the output layer by considering the difference the observed
values and the actual values.

\subsubsection{Adjusting the Synaptic Weights of the Intermediate
Layers}\label{adjusting-the-synaptic-weights-of-the-intermediate-layers}

~~~~~~~Unlike the neurons in the output layer, neurons in the
intermediate layers do not have access to the actual values. It follows
that adjustments in the intermediate layers rely on neurons in the
aftermost layers for feedback.

\subsection{Adjusting the synaptic weights of the second hidden
layer}\label{adjusting-the-synaptic-weights-of-the-second-hidden-layer}

~~~~~~~In order to train the second neural layer, we need to adjust the
weight matrix \(W_{ji}^{(2)}\) so as to minimize the error between
outputs produced by the network and the adjustment produced in the
output layer.
...
Obtained from the partial derivative of equation 3.4 with respect to
\(W_{ji})\)
...
Note that the partial derivative of parcel (ii) with respect to
\(Y_j^{(2)}\) is the value of \(W_k^{(3)}\). Thus equation 3.21 becomes:
...
~~~~~~~The value of parcel (ii) in equation 3.22 represents the synaptic
weight of neurons from the output layer that is connected to the neuron
j of the second intermediate layer. Also, the weights \(W_{ji}^{(3)}\)
have been adjusted in the previous step based on errors, which will be
used to adjust weights in the second intermediate layer. Moving on to
parcel (i) in equation 3.22, the result can be simplified by multiplying
equations 3.13 and 3.14 together, which results in the actual value of
\(δ_j^3\) defined in equation 3.16 defined above. Thus, substituting in
...
Da Silva et al., (2017) posit that the adjustment of the weight matrix
\(W_{ji}^{(2)}\) must be made in the opposite direction of the gradient
in order to minimize the error. Thus:
...
In short, we have shown that that equation 3.27 adjusts weights from
neurons in the second hidden layer by considering backpropagation errors
originating from the neurons in the output layer.

\subsubsection{Adjusting the synaptic weights of the first hidden
layer}\label{adjusting-the-synaptic-weights-of-the-first-hidden-layer}

~~~~~~~The objective of the training process of the first hidden layer
consists of adjusting the weight matrix \(W_{ji}^{(1)}\) in order to
minimize the error between the output produced by the network and the
backpropagated error originating from the adjustment of the neurons in
the second hidden layer. Thus, we have:
...
Note that the partial derivative of parcel (ii) with respect to
\(Y_j^{(2)}\) is the value of \(W_k^{(2)}\). Thus, equation 3.32
becomes:
...
~~~~~~~The value of parcel (ii) in equation 3.33 relates to the synaptic
weights of neurons from the second hidden layer. It is important to
emphasize that all the weights of \(W_{ji}^{(2)}\) were adjusted in the
previous step based on the backpropagated errors from the adjustment of
the weights of \(W_{ji}^{(2)}\), which in turn were adjusted based on
the real values of the error. Parcel (i) in equation 2.30 can be
obtained by multiplying equations 3.21 and 3.22, which will result in
the value of \(δ_j^{(2)}\) obtained in equation 3.27.
...
In short, the adjustment of the weight matrix \(W_{ji}^{(1)}\) should be
made in the opposite direction of the gradient in order to minimize the
error.
...
Therefore, expression (3.38) adjusts the weight of the neurons from the
second hidden layer, using the backpropagation of the error originated
from the neurons of the output layer

\section{Convolution Neural Network
Architecture}\label{convolution-neural-network-architecture}

~~~~~~~Convolution neural networks rely on their architecture to make
predictions from inputs. This architecture can be divided into feature
extraction and model prediction. The feature extraction phase relies on
techniques such as filtering, the stride, padding, pooling and
flattening. In order to explain how each of these techniques contributes
to the architecture of the neural network we shall rely on the image
shown in figure 3.4 below.

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{img/matrix_and_kernel} 

}

\caption{Sample Matrix and a Kernel.}\label{fig:matrix_and_kernel}
\end{figure}

\subsection{Filtering in Convolution Neural
Networks}\label{filtering-in-convolution-neural-networks}

~~~~~~~In order to understand how filtering works, we need to consider
figure 3.4. The kernel slides over the sample matrix and takes the dot
product between the kernel and a 3 by 3 matrix from rows in the sample
matrix. For example, the kernel slides from R1 to row R3 and from column
C1 to column C3 and multiplies the resulting matrix with the kernel. In
this case, the result of the dot multiplication is 12. It then slides
over and beginning form row R2 to row R4 and column C1 to column C3. The
result of this dot multiplication is also 12. The last filter happens
from row R3 to row R5 and column C1 to column c3. The result from this
dot multiplication is 17. The kernel then moves one column downwards and
takes a 3 by 3 matrix from row R1 column C2 to row R3 column C4 and
multiplies itself with this matrix to give 10. Holding the same columns,
the kernel slides from row R2 to row R4 and multiplies the resulting
matrix with itself to give 17. It then slides from row R3 to R5 and
multiplies the resulting matrix with itself to give 19. The procedure is
through rows R1 to R5 and columns C3 to C5 to give 9, 6 and 14 as the
results. The results are combined into what is called the feature map
and is shown in figure 3.5 below.

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{img/feature_map_1} 

}

\caption{Feature Map Resulting from a Kernel Filter.}\label{fig:feature_map_1}
\end{figure}

\subsection{Strides in Convolution Neural
Network}\label{strides-in-convolution-neural-network}

~~~~~~~A stride in convolution neural networks could be defined as a
step used by the filter(kernel) to slide over the input image. A stride
of 1 means that the filter moves one pixel at a time, while a stride of
2 means it moves two pixels at a time. Let us take figure 3.4 into
consideration (assuming that we have a stride of 1) and use it to
explain how strides are used in convolution neural networks. The filter
will begin in the same position as the filtering process i.e.~from R1 to
row R3 and from column C1 to column C3 and multiplies the resulting
matrix with the kernel. In this case, the result of the dot
multiplication is 12. It will then move one pixel to the right and begin
filtering from row R3 to row R5 and column C1 to column c3. The result
from this dot multiplication is 17. Moving downwards the stride will
skip one pixel downwards and begin the filtering process from column C3
to column C5 to give 9 as the result. It will then skip one row and
begin filtering from row R3 column C3 which will give 14 as the result.
In short, the matrix and kernel in figure 3.4 and a stride of 1 will
produce a feature map shown in figure 3.6 below.

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{img/strides} 

}

\caption{Image Showing The Output after a Stride of 1.}\label{fig:strides}
\end{figure}

\subsection{Padding in Convolution Neural
Networks}\label{padding-in-convolution-neural-networks-1}

~~~~~~~While this paper discussed padding, it has not touched on how
padding is used in CNNs. This is why it is important to demonstrate how
padding: -specifically zero padding, is used in convolution neural
networks. This owes to the fact that the paper will create a facial
recognition model with zero padding and no padding in order to
investigate the effect of padding in facial recognition. Zero padding in
CNNs is achieved by adding zeros around an input image before filtering
is applied to the resulting matrix in order to create a feature map.
Consider the matrix in figure 3.4, adding zeros around the matrix
produces the matrix (a) in figure 3.7 below. The matrix (a) in figure
3.7 below is then subjected to a filter using the kernel in figure 3.4
to produce the matrix(b) in figure 3.7 below. It is important to note
that the dimension of the matrix(b) in figure 3.7 below is equal to the
dimension of the sample matrix in figure 3.4 above

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{img/padding} 

}

\caption{Image Showing How Padding is Applied in CNNs.}\label{fig:padding}
\end{figure}

~~~~~~~Pooling is a technique used to reduce a feature map into a much
smaller dimension while maintaining critical features from a feature
map. Critical to the discussion is the fact that there are two main
pooling techniques used by CNNs: - max pooling and average pooling. The
max pooling technique divides the feature map into 2 by 2 matrices and
selects the maximum value from each of the divided matrices before
combining the result into a matrix. Let us consider a pooling size of 2
by 2 and a feature map shown in figure 3.8 below. Max pooling and
average pooling will divide the feature map in the following ways.
...
Max pooling combines results above by taking the maximum value from each
matrix and concatenating the result as shown in figure 3.8 below. In
contrast, rather than take the maximum value from each matrix it finds
the averages and concatenates the results as shown in the matrix called
average pooling in figure 3.8 below.
...
\caption{Image Showing MAx and Average Pooling in CNNs.}\label{fig:pooling}
\end{figure}
...
~~~~~~~Batch normalization is used by neural networks to improve the
speed and stability of the training process. Critical to the discussion
is the fact that Sergey and Szegedy (2015) have a publication on some of
the benefits of batch normalization. This owes to the fact that batch
normalization does not only improve the speed and the stability of the
training process, but also reduces the internal covariate shift. The
term internal covariate shift could be defined as a phenomenon where the
distribution of outputs of a neural network change when the model is
still being updated. That said, this technique occurs by adjusting the
values in each variable using the mean, standard deviation, scaling
parameter, and a bias. This is done using equation 3.39 below. It is
important to highlight that the scaling parameter and the bias, like the
weights of a neural network, are adjusted in order to minimize the loss
function.
...
Agrawal, A. K., Singh, N. Y. (2015). Evaluation of Face Recognition
Methods in \hangindent=40pt Unconstrained Environments. International
Conference on Intelligent Computing, Communication \& Convergence.
Bhubaneswar, Odisha, India.

Alrasheedi, F., Zhong, X., \& Huang, P.C. (2023). Padding Module:
Learning the \hangindent=40pt Padding in Deep Neural Networks. IEEE
Access, 11, 7348-7357.

Astawa, I. N. G. A., Putra, I. K. G. D., Sudarma, M., and Hartati, R. S.
(2020). KomNET: \hangindent=40pt Face Image Dataset from Various Media
for Face Recognition. Data in Brief, 31(105677): 1-5.

Ayad, W., Qays, S., Perera g. A., and Al-Naji, A. (2023). Facial
Recognition Databases: \hangindent=40pt Recent Developments and Review
of Methods. Journal of Techniques, 5(4): 95-104.

Celiktutan, O., Ulukaya, S., and Sankur, B. (2013). A Comparative Study
of Face \hangindent=40pt Landmarking Techniques. EURASIP Journal on
Image and Video Processing, 3(2013).

Chen, L., Mark, H., Lin, J., and Han, C. (2001). Why Recognition in A
Statistics-Based \hangindent=40pt Face Recognition System Should be
based on The Pure Face Portion: A Probabilistic Decision- Based Proof.
Pattern Recognition, 34 (2001): 1393-1403.

Da Silva I. N. Hernane Spatti D. Andrade Flauzino R. Liboni L. H. B. \&
Dos Reis \hangindent=40pt Alves S. F. (2017). Artificial Neural
Networks: A Practical Course. Springer International Publishing:
Imprint: Springer.

Dupuis, E., Novo, D., O'Connor, I., and Bosio, A. (2021). CNN Weight
Sharing Based on a \hangindent=40pt Fast Accuracy Estimation Metric.
Retrieved from: \url{https://hal.science/hal-03257748/document}.
Retrieved on 7 January, 2023.

Finlayson G. D. (2018). Colour and Illumination in Computer Vision.
Interface \hangindent=40pt focus, 8(4), 20180008.

Finlayson, G., and Fredembach, C. (2007). Detecting Illumination in
Images. \hangindent=40pt Retrieved from:
\url{https://www.cs.sfu.ca/~mark/ftp/Iccv07/iccv07.pdf}. Retrieved on: 3
September, 2023.

Ghimire, D., and Lee, J. (2016). Geometric Feature-Based Facial
Expression Recognition \hangindent=40pt in Image Sequences Using
Multi-Class AdaBoost and Support Vector Machines. Sensors, 13(6):
7714--7734.

Gupta, D. (2017). Comparative Evaluation of Deep Architectures for Face
\hangindent=40pt Recognition in Unconstrained Environment (FRUE).
International Journal of Advanced Research and Engineering, 6(9):
1353-1360

He, L.; He, L.; Peng, L. (2023) CFormerFaceNet: Efficient Lightweight
Network \hangindent=40pt Merging a CNN and Transformer for Face
Recognition. Appl. Sci. 13(6506): 1-14.

Hidayat, R., Wibowo, B, M., Satria, R. B., Winursito, A. (2021).
Implementation \hangindent=40pt of Face Recognition Using Geometric
Features Extraction. Jurnal Lmiah Kursor, 11(2): 83-90.

Hinton, E. G., Osindero, O., and Teh, Y. (2006). A Fast Learning
Algorithm for \hangindent=40pt Deep Belief Nets. Neural Computation,
18(7): 1527-54.

Hosseini, H., Xiao, B., Jaiswal, M., and Poovendran R. (2017). On the
Limitation \hangindent=40pt of Convolutional Neural Networks in
Recognizing Negative Images. Retrieved from:
\url{https://arxiv.org/pdf/1703.06857.pdf} Retrieved on: 29 October,
2023.

Ibrahem, H. M., and Abdulameer, H. M. (2023). Age Face Invariant
Recognition \hangindent=40pt Model Based On VGG Face Based DNN And
Support Vector Classifier. International Journal on ``Technical and
Physical Problems of Engineering. 54(15): 232-240.

Kapil, D., and Jain A. (2013). A Brief Review on Feature Based
Approaches for \hangindent=40pt Face Recognition. International Journal
of Science and Research, 6(14): 273-27.

Koo, H. J., Cho, W. S., Baek, R. N., Kim, C. M., and Park, R. K. (2018).
\hangindent=40pt CNN-Based Multimodal Human Recognition in Surveillance
Environments. Sensors 18(3040): 2-34

Koodalsamy, B., Bairavan, V., Narayanasamy, V. (2023). Face Recognition
using Deep \hangindent=40pt Learning. International Conference on Smart
Engineering for Renewable Energy Technologies (n.p.). Rajapalayam,
India. \url{https://doi.org/10.1051/e3sconf/202338705001}. Retrieved on:
29 October, 2023.

Nguyen, A.D., Choi, S., Kim, W., Ahn, S., Kim, J., \& Lee, S. (2019).
Distribution \hangindent=40pt Padding in Convolutional Neural Networks.
In 2019 IEEE International Conference on Image Processing (ICIP)
(pp.~4275-4279).

Ortiz, E. G., \& Becker, B. C. (2014). Face recognition for web-scale
datasets. \hangindent=40pt Computer Vision and Image Understanding, 118,
153--170.

Parkhi, M, O., Vedaldi, A., and Zisserman, A. (2015). Deep Face
Recognition. Retrieved \hangindent=40pt from:
\url{https://www.robots.ox.ac.uk/~vgg/publications/2015/Parkhi15/parkhi15.pdf}.
Retrieved on: 9 September, 2024.

Proyecto, F., C. (2010). Face Recognition Algorithms. University of the
Basque \hangindent=40pt Country. Retrieved from:
\url{https://www.ehu.eus/ccwintco/uploads/e/eb/PFC-IonMarques.pdf}.
Retrieved on: 3 September, 2023.

Ranjan, A., Behera, N. J., and Reza. M. (2020). A Parallel Approach for
Real-Time Face \hangindent=40pt Recognition from a Large Database.
Retrieved from: \url{https://arxiv.org/pdf/2011.00443.pdf}. Retrieved
on: 3 September, 2023.

Reddy, M. D., \& Reddy, S. N. (2019). Effects of Padding on LSTMS and
CNNS. Retrieved \hangindent=40pt from:
\url{https://arxiv.org/abs/1903.07288}. Retrieved on: 28 August, 2024.

Rio, L. A., Martin, M., Perera-Lluna, A., \& Saidi, R. (2020). Effect Of
Sequence \hangindent=40pt Padding On The Performance of Deep Learning
Models in Archaeal Protein Functional Prediction. Scientific Reports,
10(14634): n.p.

Roza, D., and Mohsen S. (2021). Artificial Neural Network Systems.
International \hangindent=40pt Journal of Imaging and Robotics,
21(2):13-25.

Rusia, K. M., Singh K. D. (2023). A Comprehensive Survey on Techniques
to Handle \hangindent=40pt Face Identity Threats: Challenges and
Opportunities. Multimedia Tools and Applications (2023) 82:1669--1748.

Shah, A., Shinde, S., Kadam, E., Shah, H., Shingade, S. (2016). Deep
Residual \hangindent=40pt Networks with Exponential Linear Unit.
Retrieved from: \url{https://arxiv.org/pdf/1604.04112.pdf}. Retrieved
on: 1 February, 2024

Shalini, T., Jabbar, A. S., Vamsi, S, T., Prakash, B. P., Kalyan, S. Y.
(2023). \hangindent=40pt Face Recognition Based Attendance System Using
CNN Architecture. International Journal for Innovative Engineering and
Management Research, 12(4): 232-238.

Schroff, F., Kalenichecnko, D., and Philbin, J. (2015). FaceNet: A
Unified \hangindent=40pt Embedding for Face Recognition and Clustering.
Retrieved from: \url{https://arxiv.org/pdf/1503.03832}. Retrieved on 15
September, 2024.

Singh, M., Chawla, M., Singh, R., Vatsa, M., \& Chellappa, R. (2019).
Disguised \hangindent=40pt Faces in the Wild 2019. In 2019 IEEE/CVF
International Conference on Computer Vision Workshop (ICCVW)
(pp.~542-550).

Singh, M., Singh, R., Vasta, M., Ratha, N., and Chellappa, R. (2018).
Recognizing \hangindent=40pt Disguised Faces in The Wild. IEEE
Transactions on Biometrics, Behavior, and Identity Science, 1(2):
97-108.

Singh, S., and Prasad, S. (2018). Techniques and Challenges of
Recognition: A \hangindent=40pt Critical Review. 8th International
Conference on Advances in Computing and Communication. Rajagiri School
of Engineering \& Technology, Kochi, India. 143 (2018) 536--543.

Struc, v. and Pavesic, N. (2009). A Case Study on Appearance Based
Feature \hangindent=40pt Extraction Techniques and Their Susceptibility
to Image Degradations for the Task of Face Recognition. World Academy of
Science, Engineering and Technology, 30(2009): 806-814.

Sun, Q., and Redei A. (2022). Knock Knock, Who's There: Facial
Recognition using \hangindent=40pt CNN-based Classifiers. International
Journal of Advanced Computer Science and Applications, 13(1): 9-16.

Tang, H., Ortis, A., \& Battiato, S. (2019). The Impact of Padding on
Image \hangindent=40pt Classification by Using Pre-trained Convolutional
Neural Networks. In E. Ricci, S. Rota Bulò, C. Snoek, O. Lanz, S.
Messelodi, \& N. Sebe (Eds.), Image Analysis and Processing -- ICIAP
2019 (pp.~337--344). Cham: Springer International Publishing.

Tarnate, J. M., Devaraj, M., and Goma, C, J. (2020). Overcoming the
Vanishing Gradient \hangindent=40pt Problem of Recurrent Neural Networks
in The Iso 9001 Quality Management Audit Reports Classification.
International Journal of Scientific and Technology Research, 9(3): 6683-
6686

The World Economic Forum. (2023). Future of Jobs Report. Retrieved
from:\\
\hangindent=40pt
\url{https://www3.weforum.org/docs/WEF_Future_of_Jobs_2023.pdf}.
Retrieved on: 12 September, 2023.

Vicario, G., and Coleman, S. (2018). A Review of Data Science in
Business and Industry \hangindent=40pt and a Future Review. Applied
Stochastic Models in Business and Industry, 36(1), 6-18.

Viegas, M. A., Simoes, R., Thanekar, N., Barnerjee, S., Dicholkar, R.
(2019). Different \hangindent=40pt Approaches of Face Recognition.
International Journal of Engineering Research \& Technology, 8(6):
933-934.

Wang, D., Wang, H., Sun, J., Xin, J, and Luo, Y. (2021). Face
Recognition in Complex \hangindent=40pt Unconstrained Environment with
an Enhanced WWN Algorithm. Journal of Intelligent Systems, 2022(30):
18-39.

Wu, Z., Xiamixiding, R. Sjjanhar, A., Chen, J., and Wen, Q. (2018).
Image \hangindent=40pt Appearance-Based Facial Expression Recognition.
International Journal of Image and Graphics 18(2): 1 -12.

Xiaobo, Q., Chenxu, W., Ying, S., Hui, Q., Kaige, D., and Xiaobin, W.
(2023). A \hangindent=40pt Convolutional Neural Network Face Recognition
Method Based on BiLSTM and Attention Mechanism. Computational
Intelligence and Neuroscience, 2023(2501022): 1-14.

Yan, Y., Lee, F., Wu, X., and Chen, Q. (2018). Face Recognition
Algorithm Using Extended \hangindent=40pt Vector Quantization Histogram
Features. PLos One 13(1): n.p.

Yang, M., Kriegman, D, J., Ahuja, N. (2002). Detecting Faces in Images:
A Survey. IEEE \hangindent=40pt Transactions on Pattern Analysis and
Machine Intelligence, 24(1): 1-25.

Zhang, X., Xv, C., Shen, M., He, X., and Du, W. (2018). Survey of
Convolutional Neural \hangindent=40pt Network. Advances in Intelligent
Systems Research, 147(2018): 93-97.

Zhang, Z., Hu, R., and Hu, W. (2022). Application of Artificial Neural
Network Algorithm \hangindent=40pt in Facial Biological Image
Information Scanning and Recognition. Contrast Media \& Molecular
Imaging, 2022(1142682): n.p.
